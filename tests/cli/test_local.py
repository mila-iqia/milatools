...
def _test_stdouterr(func, capsys, file_regression):
    ...
    try:
        ...
        out = out if out else ""
        err = err if err else ""
        file_regression.check(f"{captured.out}:::::\n{captured.err}=====\n{out}^^^^^\n{err}^^^^^\n")
    ...
    if cmd in [_FAKE_CMD, _FAIL_CODE_CMD]:
        def _catch_exc():
            with pytest.raises(CommandNotFoundError) as exc_info:
                func()
            exc_info.value.args = (exc_info.value.args[0].replace("FAKE", ""), *exc_info.value.args[1:])
...
def test_popen(cmd, capsys, file_regression):
    _test_stdouterr(lambda: Local().popen(*cmd, stdout=PIPE, stderr=PIPE).communicate(), capsys, file_regression)
