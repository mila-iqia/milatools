...
def linux_ssh_config(
    ...
    start_pos = input_stream.seek(0, 1)
    prompt_inputs = [
        "y",  # do you have an account on the Mila cluster?
        mila_username,  # What's your username on the Mila cluster?
        "y",  # Do you also have a DRAC account?
        drac_username,  # username on DRAC
        "y",  # Accept modifying the SSH config file.
    ]
    end_pos = input_stream.write("\n".join(prompt_inputs) + "\n")
...
class TestSetupMilaSSHAccess:
    ...
    This is what `init_command` does for the Mila cluster (after the SSH config is done).
    ...
    async def test_mila_access_already_setup(
        ...
        # Make sure that there aren't any active connections to the Mila cluster before
        # the test runs.
        assert _check_shared_connection_returncode("mila") != 0
        ...
        # Using my real SSH directory, we should have access to the Mila login nodes.
        assert (
