...
async def test_code_default_mem_flag(
    ...
    alloc: list[str],
    expected_mem_default: bool,
):
    """Test that --mem=8G is added to alloc by default when no mem flag is present."""
    from milatools.cli import code as code_module
    ...
    monkeypatch.setattr(code_module, RemoteV2.__name__, mock_remote_v2_cls)

    monkeypatch.setattr(code_module, check_disk_quota.__name__, AsyncMock())
    monkeypatch.setattr(
    ...
    async def _mock_salloc(login_node, salloc_flags, job_name):
        ...
        return Mock(spec=ComputeNode, hostname="cn-a001", job_id=12345)

    monkeypatch.setattr(code_module, salloc.__name__, _mock_salloc)
